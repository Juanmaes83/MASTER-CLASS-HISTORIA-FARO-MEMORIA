...
async function noHorizontalOverflow(page){const g=await page.evaluate(()=>({sw:document.documentElement.scrollWidth,cw:document.documentElement.clientWidth}));expect(g.sw).toBeLessThanOrEqual(g.cw)}
...
test('Mobile B1 Guided and Group stay inside a 390px viewport',async({page})=>{await page.setViewportSize({width:390,height:844});await mockAudio(page);await ready(page);await workshop(page);await expect(page.locator('.b1-home')).toBeVisible();await noHorizontalOverflow(page);await page.locator('[data-b1="guided"]').click();await expect(page.locator('.b1-prompt-stage')).toBeVisible();await expect(page.locator('#b1-record-guided')).toBeVisible();await noHorizontalOverflow(page);await page.locator('#b1-back').click();await page.locator('[data-b1="group"]').click();await expect(page.locator('.group-config')).toBeVisible();await expect(page.locator('#b1-start-group')).toBeVisible();await noHorizontalOverflow(page)});